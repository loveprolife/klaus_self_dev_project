({
	doInit : function(component, event, helper) {
		component.set("v.pdfUrl", "/apex/CreditLimitApplicationReport?ID="+component.get("v.recordId"));//v 是组件属性集的值提供程序
		var action = component.get("c.getCreditLimitApplication");//componnet.get("c.testAction"): 此逻辑代表获取后台apex controller中的 testAction方法，用于和后台交互操作，返回类型为Action对象变量，
		var CreditLimitApplicationId = component.get("v.recordId");
		console.log('CreditLimitApplicationId:'+CreditLimitApplicationId);
        //匹配设备型号
		...
		action.setParams({
			CreditLimitApplicationId:CreditLimitApplicationId
		});
