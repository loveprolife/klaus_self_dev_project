...
import getInitData from '@salesforce/apex/NewPromoterDailyReportThailandController.getInitData';
import saveData from '@salesforce/apex/NewPromoterDailyReportActionController.saveDataThailand';
...
import refreshData from '@salesforce/apex/NewPromoterDailyReportThailandController.refreshData';
...
import createData from '@salesforce/apex/NewPromoterDailyReportThailandController.createData';
...
import RetailName from '@salesforce/label/c.RetailName';
import RetailName2 from '@salesforce/label/c.RetailName2';
...
export default class NewPromoterDailyReportThailandLwc extends LightningNavigationElement {
    /**
    ...
    get shopOptions() {
        ...
        if (this.responsiblePersonList) {
            console.log('wwww' + JSON.stringify(this.responsiblePersonList));
            for (let i = 0; i < this.responsiblePersonList.length; i++) {
                var rp = this.responsiblePersonList[i];
                options.push({label: rp.StoreId__r.Name, value: rp.StoreId__c});
            }
        }
        return options;
    };
    ...
    get recordDisabled() {
        // return this.record.Status__c != 'New' || this.record.Id == null || this.record.Status__c != 'Submitted';
        return false;
        // return true;
    }
    ...
    connectedCallback() {
        // console.log('WWWW isMobile' + this.isMobile);
        ...
        judgeCountry().then(data => {
            ...
            if (data.isSuccess) {
                ...
                console.log('wwwwdeviceId:', this.deviceId);
                ...
                getInitData({
                    ...
                    if (data.isSuccess) {
                        ...
                        console.log('data.data>>>>>>>1'+JSON.stringify(data.data));
    ...
    handleBack(){
        ...
        // window.close();
        // // 为了防止出现无法关闭的提示，可以在window.close之后加上self.close()
        // self.close();
        // this.goToVf('/lightning/n/Daily_Report_List')
        // this.goToVf('/lightning/n/Daily_Report_Thailand')
        this.goToVf('/lightning/cmp/c__dailyReportListViewCmp')
        // this.goToVf('/lightning/n/Daily_Reports')
    }
    ...
    saveReportCore(isSubmit, type) {
        ...
        console.log('Status----------'+this.record.Status__c);
        ...
        saveData({
            ...
            if (data.isSuccess) {
                ...
                console.log('data.data>>>>>>>2'+JSON.stringify(data.data));
    ...
    dailySaleSave() {
        ...
        saveDailySalesData({
            ...
            if (data.isSuccess) {
                ...
                console.log('data.data>>>>>>>3'+JSON.stringify(data.data));
    ...
    goodsReceivingSave() {
        ...
        saveGoodsReceivingData({
            ...
            if (data.isSuccess) {
                ...
                console.log('data.data>>>>>>>4'+JSON.stringify(data.data));
    ...
    intelligenceSave() {
        ...
        saveIntelligenceData({
            ...
            if (data.isSuccess) {
                ...
                console.log('data.data>>>>>>>5'+JSON.stringify(data.data));
        ...
        if (this.retailNumberList && this.retailNumberList.length>0) {
            for (let i = 0; i < this.retailNumberList.length; i++) {
                ...
                // 2024-01-08 wfc Daily Sales里的Unit Price这个字段不能为0,印尼的这个字段除了不能提报0以外填的数字不能少于七位数
                    ...
                    // 验证文件必须上传
                    let fileList = this.retailNumberHaveFileIdMap[retailNumber.Id];
        ...
        if (this.stockList && this.stockList.length>0) {
            for (let i = 0; i < this.stockList.length; i++) {
                ...
                if (stock.Product_Model__c == null || stock.Product_Model__c == '') {
                    ...
                    resp.alltrue = false;
                    resp.msg = this.stock.Quantities__c;
                    this.lwcName = this.label.PromoterDailyReport_Stock;
                    return resp;
                }
            }
    ...
    handleRefreshData() {
        ...
        refreshData({
            ...
            if (data.isSuccess) {
                ...
                console.log('data.data>>>>>>>6'+JSON.stringify(data.data));

                if (data.data.record && data.data.record.Id) {
                    // this.recordId = data.data.record.Id;
                    this.goToComponent('c__LWCWrapper',{
                        'lwcName' : 'newPromoterDailyReportThailandLwc',
...
//     /**-----------------Button function---------------*/
                ...
                if(selectRecord.Name == null) {
                    ...
                    for (let i = 0; i < this.stockList.length; i++) {
                        ...
                        if (item != '' && item == selectRecord.Id) {
                            ...
                            this.stockList[i].Product_Model__c = selectRecord.Id;
                        }
                    }
                } else {
    ...
    get recordDatereadonly() {
        console.log('wwwwrecordDatereadonly' + this.isTitleReadOnly);
