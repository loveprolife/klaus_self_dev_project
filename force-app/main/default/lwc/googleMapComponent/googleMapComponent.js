...
export default class GoogleMapComponent extends LightningNavigationElement{
    ...
    connectedCallback(){
        if (navigator.geolocation) {
            navigator.geolocation.getCurrentPosition(position => {
                ...
                // 构造带参数的 VF 页面 URL
                this.googleMapUrl = `/apex/GoogleMapTest?param1=${encodeURIComponent(this.currentLat)}&param2=${encodeURIComponent(this.currentLong)}`;
            }, error =>{
                this.googleMapUrl = `/apex/GoogleMapTest`;
                switch(error.code) {
    ...
    handleMessage(event) {
        ...
        }else if(event.data.action === 'updateStore'){
            console.log('wwwww---修改门店--' + event.data.shopId);
            this.isModalOpen = true;
            // 查询门店信息
            searchShop({
                ...
                this.record = result;
                this.storeAddress = this.record['Address1__c'];
                if(this.record.Sales_Region__c.includes('Japan')){
                    this.isJapan = true;
                }else {
                    this.isJapan = false;
                }
