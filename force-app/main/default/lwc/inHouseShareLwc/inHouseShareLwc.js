...
export default class InHouseShareLwc extends LightningNavigationElement {

    @api lwcName = PromoterDailyReport_IN_HOUSE_SHARE;
            ...
            "hisensShare": {
                get() {
                    ...
                    for (let index = 0; index < brandLen; index++) {
                        let number = Number(items[index].Stock__c);
    ...
    connectedCallback() {
        ...
        if (this.recordId) {
            getInitData({
                ...
                this.recordTypeId = data.data.InHouseShare;
    ...
    brandItemAdd(event) {
        ...
        console.log('inHouseShare ——> brandItemAdd ——> result.flag：' + result.flag);
        if (result.flag) {
            let productLine = '';
    ...
    itemAddHandler(event) {
        let result = this.addCheck();
        console.log('inHouseShare ——> saveData ——> result.flag：' + result.flag);
        if (result.flag) {
    ...
    deleteLastOne() {
        console.log('Stock Share deleteLastOne ----：');
        let lastReocrdIsEmpty = true;
            ...
            if (lastRecordItemList.length > 0) {
                ...
                if (!lastItem.HisenseFlag__c) {
                    ...
                    if (typeof lastItem.isUpdatedFile != "undefined" && lastItem.isUpdatedFile) {
                        ...
                        lastReocrdIsEmpty = false;
                    } else if (this.isFilledOut(lastItem.Stock__c)) {
                        lastReocrdIsEmpty = false;
    ...
    saveCheck() {
        ...
        console.log('Stock Share saveCheck ----：');
        if (this.recordList.length > 0) {
        ...
        console.log('Stock Share addCheck ----：');
        ...
        for (let index = 0; index < this.recordList.length; index++) {
            ...
            for (let i = 0; i < brandLen; i++) {
                ...
                if (branItem.HisenseFlag__c) {
                    if (!this.isFilledOut(branItem.Stock__c)) {
                        result.flag = false;
                        missFields.add(this.fields.Stock__c);
                    }
                } else {
