...
import IconClose from '@salesforce/resourceUrl/IconClose';
...
import getNoteByTaskId from '@salesforce/apex/CreateNewInspectionController.getNoteByTaskId';

export default class CreateNewInspectionPlanLwc  extends LightningNavigationElement {
    @api recordId;
    @api yearTemp;
    @api monthTemp;
    @api ownerIdTemp;
    ...
    @track currentMonth = '';
    @track currentYear = '';
    @track draggedStoreId = null;
    @track showFilter = false;

    @track showMoreTask = false;
    @track moreTaskStyle;
    @track moreTaskDate;
    @track moreTaskList;
    @track moreIndex;
    @track moreTaskCanDelete;
        ...
        saveData({
            ...
            if (result.isSucess) {
                if (result.canCreate) {
                    ...
                    // 处理日期
                    const date = new Date(this.ShopPlanData.Inspection_Year__c, this.ShopPlanData.Inspection_Month__c - 1, 1);
                    const offset = date.getTimezoneOffset() * 60000; // 毫秒偏移量
                    this.currentDate = new Date(date.getTime() - offset);
    ...
    notSave() {
        ...
        console.log('wwwthis.ShopPlanData-----'+this.ShopPlanData.Inspection_Year__c);
        console.log('wwwthis.ShopPlanData-----'+this.ShopPlanData.Inspection_Month__c);
        //调用后台逻辑
            ...
            haveRepeatingData({
                ...
                if (result.isSucess) {
                    if (result.canCreate) {
                        ...
                        if(this.salesRegion == 'Hisense Japan'){
                        ...
                        this.noAddStoreList = result.noAddStoreList;
    ...
    connectedCallback() {
        ...
        getData({
            ...
            if (result.isSucess) {
                ...
                this.ShopPlanData = result.ShopPlanData;
                console.log('wwwwwfffff----' + JSON.stringify(result.ShopPlanData));
                this.monthplans = result.MonthPlans;
                // console.log('wwwww----' + JSON.stringify(result.MonthPlans));
                this.dayOptions = result.dayAndWeekDate;
                this.myDate = result.nowDate;
                // 处理日期
                const date = new Date(this.ShopPlanData.Inspection_Year__c, this.ShopPlanData.Inspection_Month__c - 1, 1);
    ...
    showTooltipView(event){
        ...
        const isCanDelete = event.currentTarget.dataset.iscandelete;
        const taskId = event.currentTarget.dataset.taskid;
        const shopId = event.currentTarget.dataset.shopid;
        const shopName = event.currentTarget.dataset.shopname;
        const taskDate = event.currentTarget.dataset.taskdate
        ...
        this.modalOpenStyle = 'top: ' + (mouseY - distanceFromTop - 140)  + 'px;' + 'left: ' + mouseX + 'px; padding:0;border: 1px solid #b5b5b5; z-index: 30';
        ...
        .then(result => {
            if(result){
                ...
                this.task = {
                    Id: taskId,
                    Shop__c: shopId,
                    Inspection_Date__c: taskDate,
                    Note__c: result.Note__c != null ? result.Note__c : '',
                };
            }
        }).catch(error => {})

        this.showDisable = isCanDelete == 'false';
        this.showButton = isCanDelete == 'true';
    }
    ...
    handleScroll = () => {
        ...
        this.scrollTimer = setTimeout(() => {
            ...
            if (scrollTop === 0) {
                ...
                const header1 = this.template.querySelector('.my-thead1');
                ...
                header1.style.top = (window.scrollY - this.heightTemp) + 'px';
    ...
    renderedCallback(){
        ...
        if(element && element1 && this.firstInit){
            const rect = element.getBoundingClientRect();
            const distanceFromTop = rect.top;

            const rect1 = element1.getBoundingClientRect();
            const distanceFromTop1 = rect1.top;
            this.heightTemp = distanceFromTop1 - distanceFromTop

            this.firstInit = false;
        }
    }

    isExceedTimelimitNumber = 0;
    isPlanNumber = 0;
    isActualNumber = 0;
    isUnscheduledNumber = 0;
    // 处理日历数据
    updateCalendar() {
        console.log('qqqqqqqqqqqqqq----' + JSON.stringify(this.currentDate));
        ...
        this.isExceedTimelimitNumber = 0;
        this.isPlanNumber = 0;
        this.isActualNumber = 0;
        this.isUnscheduledNumber = 0;

        // 处理查询的任务数据
        let taskMap = {};
        this.monthplans.forEach(item => {
            item.dayOptions.forEach(dayInfo=> {
                if(dayInfo.taskId != null && dayInfo.taskId != '' && dayInfo.taskId != undefined){
                    dayInfo.shopName = item.shopName;
                    dayInfo.shopId = item.shopId;
                    if(taskMap[dayInfo.taskDate] != null && taskMap[dayInfo.taskDate] != '' && taskMap[dayInfo.taskDate] != undefined){
                        taskMap[dayInfo.taskDate] = [
                            ...taskMap[dayInfo.taskDate],
                            dayInfo]
                    }else {
                        taskMap[dayInfo.taskDate] = [dayInfo];
                    }
                    dayInfo.cssClass = ' slds-truncate rounded-rect slds-var-m-left_xx-small slds-var-m-right_xx-small slds-var-m-bottom_xx-small ';
                    // 统计task数据
                    if(dayInfo.isPlan){
                        this.isPlanNumber += 1;
                        dayInfo.cssClass += ' rounded-rect-green';
                    }else if(dayInfo.isActual){
                        this.isActualNumber += 1;
                        dayInfo.cssClass += ' rounded-rect-blue ';
                    }else if(dayInfo.isUnscheduled){
                        this.isUnscheduledNumber += 1;
                        dayInfo.cssClass += ' rounded-rect-orange ';
                    }
                    if(dayInfo.isExceedTimelimit){
                        this.isExceedTimelimitNumber += 1;
                        dayInfo.cssClass += ' rounded-rect-pink ';
                    }
                }
            })
        })
        ...
        for (let i = 1; i <= lastDay.getDate(); i++) {
            ...
            const offsetNew = dateNew.getTimezoneOffset() * 60000; // 毫秒偏移量
            const localDateNew = new Date(dateNew.getTime() - offsetNew);
            const formattedDateNew = localDateNew.toISOString().split('T')[0];

            let isCanDelete = true;
            if(formattedDate < formattedDateNew){
                isCanDelete = false;
            }

            // 当前日期的任务数过多后，折叠显示
            let taskListLength = taskMap[formattedDate] ? taskMap[formattedDate].length : 0;
            let taskList = taskMap[formattedDate] ? taskMap[formattedDate] : [];
            let taskListTemp = taskMap[formattedDate] ? taskMap[formattedDate] : [];
            let isFoldable = false;
            let moreNumber = 0;
            if(taskListLength > 3){
                isFoldable = true;
                moreNumber = taskListLength - 2;
                taskListTemp = taskList.slice(0, 2);
            }
            // 根据日期获取处理的任务数据，添加在日历上
            days.push({ 
                day: i, 
                date: formattedDate,
                isCurrentMonth: true,
                isToday: this.isSameDay(date, new Date()),
                taskListTemp: taskListTemp,
                taskList: JSON.stringify(taskList),
                isCanDelete: isCanDelete,
                cssClass: this.isSameDay(date, new Date()) ? ' calendar-day-today ' : ' calendar-day ',
                cssClassNumber: this.isSameDay(date, new Date()) ? ' calendar-today-number ' : '',
        ...
        if(this.moreIndex != null && this.moreIndex != '' && this.moreIndex != undefined){
            this.moreTaskList = JSON.parse(this.calendarDays[this.moreIndex].taskList); 
        }
    }
    ...
    handleDragStart(event) {
        this.draggedStoreId = event.target.dataset.storeid;
    }
    ...
    handleDayDrop(event) {
        ...
        this.moreIndex = event.currentTarget.dataset.index;
        ...
        if(dateStr < formattedDate){
            flag = false;
            errorMsg = this.label.Inspection_Task_Create_Limits;//You cannot create tasks for dates before today.
        } else if(this.noAddStoreList != undefined){
            this.noAddStoreList.forEach(item =>{
                if(this.draggedStoreId == item){
                    flag = false;
                    errorMsg = this.label.Inspection_Task_Create_Limits4;//The floor walker is not responsible for this store anymore.
                }
            })
        }
        if (dateStr && this.draggedStoreId && flag) {
            this.createCalendarEvent(this.draggedStoreId, dateStr);
        }else {
            if(errorMsg != undefined){
                this.dispatchEvent(new ShowToastEvent({
                    title: 'error',
    ...
    // 拖拽后新建
    createCalendarEvent(storeId, dateStr) {
        addPlanTask({
            shopId : storeId, 
        ...
        .then(result => {
            if(result.isSuccess){
                this.monthplans.forEach(item => {
                    ...
                    if(item.shopId == storeId){
                        ...
                        item.dayOptions.forEach(day => {
                            ...
                            // 定位到当前taskId
                            if(day.taskDate == dateStr){
                                day.taskId = result.task.Id;
                                day.status1 = '〇';
                                day.isPlan = true;
                                day.isExceedTimelimit = result.isExceedTimelimit;
                                day.taskDate = dateStr;
                        ...
                        if(flagStore){
                            // 刷新计数
                            this.isPlanNumber = this.isPlanNumber + 1;
                        }
                    }
                });
                this.updateCalendar();
                ...
                this.dispatchEvent(new ShowToastEvent({
                    title: 'Error',
                    message: JSON.stringify(result.errorMsg),
                    variant: 'Error',
                })); 
            }
        }).catch(error => {
            this.dispatchEvent(new ShowToastEvent({
                title: 'Error',
                message: 'Add failure!' + JSON.stringify(error),
                variant: 'Error',
            }));
        })
    }
    ...
    previousMonth() {
        ...
        this.ShopPlanData.Inspection_Year__c = this.currentYear;
        this.ShopPlanData.Inspection_Month__c = this.currentDate.getMonth() + 1;
        this.callApexHaveRepeatingDataMethod();  
    }
    ...
    handleDeleteTask(event){
        ...
        this.calendarDays[index].spinner = true;
        deleteRecord({
            recordId : taskId, 
        })
        .then(result => {
            ...
            this.calendarDays[index].spinner = false;
        }).catch(error => {
            this.dispatchEvent(new ShowToastEvent({
                title: 'Error',
                message: 'Delete failure!' + JSON.stringify(error),
    ...
    handleDeleteMoreTask(event){
        event.preventDefault();
        event.stopPropagation();
        const taskId = event.target.dataset.taskid;
        const shopId = event.target.dataset.shopid;
        this.isShowMoreTaskSpinner = true;
        deleteRecord({
            recordId : taskId, 
        })
        .then(result => {
            if(result){
                ...
                this.updateCalendar();
                // this.moreTaskList = JSON.parse(this.calendarDays[this.moreIndex].taskList);   
                this.isShowMoreTaskSpinner = false;
            }
        }).catch(error => {
            this.dispatchEvent(new ShowToastEvent({
                title: 'Error',
    ...
    handlerShowMoreTask(event){
        ...
        let mouseY = event.clientY; // 垂直坐标
        // if(mouseX > 1430){
        //     this.moreTaskStyle = 'top: ' + (mouseY - distanceFromTop - 140)  + 'px;' + 'left: 1600px; padding:0;border: 1px solid #b5b5b5; z-index: 20';
        // }else {
        //     this.moreTaskStyle = 'top: ' + (mouseY - distanceFromTop - 140)  + 'px;' + 'left: ' + mouseX + 'px; padding:0;border: 1px solid #b5b5b5; z-index: 20';
        // }

        // 处理数据
        const isCanDelete = event.currentTarget.dataset.iscandelete;
        this.moreIndex = event.currentTarget.dataset.index;
        this.moreTaskList = JSON.parse(event.currentTarget.dataset.tasklist);
        this.moreTaskCanDelete = isCanDelete == 'true';
        const taskdate = event.currentTarget.dataset.taskdate;
        const date = new Date(taskdate);
        // 获取星期几
        let weekDaysTemp = [];
        if(this.salesRegion == 'Hisense Japan'){
            weekDaysTemp = ['月', '火', '水', '木', '金', '土', '日'];
        }else {
            weekDaysTemp = ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'];
        }
        const week = weekDaysTemp[date.getDay()];
        const day = date.getDate();
        ...
        this.moreTaskDate = week + ', ' + this.currentMonth + ' ' + day;

        this.showTooltipFlag = false;
        this.showMoreTask = true;
    }
    ...
    // 跳转到旧页面
    editView(){
        this.goToLwc('createNewInspection', {
    ...
    showCanDelete(event){
        event.stopPropagation();
        const index = event.target.dataset.index;
        this.calendarDays[index].isShowDelete = true;
    }
    ...
    hideCanDelete(event){
        event.stopPropagation();
        const index = event.target.dataset.index;
        this.calendarDays[index].isShowDelete = false;
    }
