      ...
      video.addEventListener("play", () => {
        if(this.isFirstPlay){
          ...
          saveCheckFileDetail({
            ...
            contentDocumentId : this.currentVideoId,
            contentDocumentTitle : this.currentVideoTitle,
            contentDocumentUrl : this.currentVideoUrl,
          }).then(result => {
  ...
  getDocuments(result) {
    ...
    if (result.data) {
      if (result.data.length) {
        this.videoMap = Object.assign({}, result.data);
        this.currentVideoCount = 0;
        this.totalVideos = result.data.length;
        this.showNavigation = !!(result.data.length - 1);
        this.hasNoVideos = false;
      } else {
        this.showNavigation = false;
        this.totalVideos = result.data.length;
