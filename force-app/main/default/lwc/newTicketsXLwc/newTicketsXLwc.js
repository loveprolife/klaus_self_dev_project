...
import getInitData from '@salesforce/apex/NewTicketsXController.getInitData';
import saveData from '@salesforce/apex/NewTicketsXController.saveData';
import handlerRemove from '@salesforce/apex/NewTicketsXController.handlerRemove';

export default class NewTicketsXLwc extends LightningNavigationElement {
    ...
    connectedCallback() {
        console.log('wwwwTicket' + this.recordId);
        ...
        if (result.flag) {
            ...
            saveData({
                ...
                if (resp.isSuccess) {
                    ...
                    console.log('wwww1=>>>>>>>>' + JSON.stringify(this.recordList));
                ...
                console.log('wwww=>>>>>>>>' + JSON.stringify(this.recordList));
            }).catch(error => {
                this.catchError(error);
                this.isShowSpinner = false;
            })
    ...
    addCheck() {
        ...
        console.log('wwwwaddCheck' + JSON.stringify(this.recordList));
        if (this.recordList.length == 0) {
            return result;
        } else {
            for (let index = 0; index < this.recordList.length; index++) {
                ...
                if ((this.isShowDepartment) && (!this.isFilledOut(this.recordList[index].AssignedTo__c) && !this.isFilledOut(this.recordList[index].Department__c))) {
                    ...
                    console.log('wwwwaddCheckfalse6 -- ' + index);
                }
            }
