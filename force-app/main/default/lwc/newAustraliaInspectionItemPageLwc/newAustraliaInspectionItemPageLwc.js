 ...
 * @LastEditors: TJP
 * @LastEditTime: 2025-09-23 17:04:35
 * @Description: 
...
export default class NewAustraliaInspectionItemPageLwc extends LightningNavigationElement {
    ...
    connectedCallback() {
        ...
        console.log('AustraliaproductLine' + this.productLine);
        console.log('AustraliarecordId' + this.recordId);
        console.log('AustraliarecordItemId' + this.recordItemId);
        console.log('AustraliastoreId' + this.storeId);
        console.log('Australiastatus' + this.status);
        console.log('Australiasubmit' + this.submit);
        console.log('AustraliafieldRed' + this.fieldRed);
        console.log('AustraliaresultInit' + JSON.stringify(this.resultInit));
    ...
    handleChangeProductOption(event) {
        ...
        if (selectRecord == undefined || selectRecord == '' || selectRecord == "") {
            ...
            this.treeData[index].parent.Response__c = selectRecord.Id;
        }
        // 根据响应条件处理展示评论或者上传文件选项
        this.handleHasFileAndCommentRequired(this.treeData[index]);
    }
    ...
    handleChangeProductLine(event) {
        let index = event.target.dataset.index;
        let value = event.target.value;
        console.log('WAWAWA' + value);
        this.treeData[index].parent.Product_Line__c = value;
        this.treeData[index].parent.Response__c = '';
    ...
    dataFormatItem(data){
        ...
        if(questionType == 'Yes/No'){
            ...
            data.object = true;
            data.showProductLine = data.CheckItem__r.Object_Api_Name__c == 'Product__c' ? true : false;
    ...
    handleDelRelate(event){
        ...
        if(this.treeData[index].children.length && indexItem >= 1){
            let indexItem = event.target.dataset.index1;
