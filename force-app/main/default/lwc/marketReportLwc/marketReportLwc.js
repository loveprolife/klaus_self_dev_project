...
import getInitData from '@salesforce/apex/MarketReportController.getInitData';
import saveData from '@salesforce/apex/MarketReportController.saveData';
import saveDailySalesData from '@salesforce/apex/MarketReportController.saveDailySalesData';
import saveGoodsReceivingData from '@salesforce/apex/MarketReportController.saveGoodsReceivingData';
import saveIntelligenceData from '@salesforce/apex/MarketReportController.saveIntelligenceData';
import refreshData from '@salesforce/apex/MarketReportController.refreshData';
import autoShowProductDes from '@salesforce/apex/MarketReportController.autoShowProductDes';
import getProductSpiv from '@salesforce/apex/MarketReportController.getProductSpiv';
import judgeCountry from '@salesforce/apex/MarketReportController.judgeCountry';
import createData from '@salesforce/apex/MarketReportController.createData';
...
export default class MarketReportLwc extends LightningNavigationElement {
    /**
     * 初始化 Market__c 标签
     */
    @track marketInfo = {
    ...
    @wire(getObjectInfo, { objectApiName: 'Market__c' })
    wiredMarketInfo({ error, data }) {
        if (data) {
            ...
            console.log(error);
            this.showError('Market__c getInformation error');
        }
    }
    /**
    ...
    //             Brands__c: data.fields.Brands__c.label,
    //             Product__c: data.fields.Product__c.label,
    ...
    lwcName = this.label.PromoterDailyReport_Market_Report;
    @track isShowSpinner
    @api recordId;
    @track hisenseAccId;
    ...
    //     var options = [];
    //     if (!this.reportDateOptionList) {
    //         return options;
    //     }

    //     if (this.record.Report_Date__c && this.reportDateOptionList.filter(obj => obj==this.record.Report_Date__c).length==0) {
    //         options.push({label: this.record.Report_Date__c, value: this.record.Report_Date__c});
    //         return options;
    //     }
    //     if (this.reportDate && this.reportDateOptionList.filter(obj => obj==this.reportDate).length==0) {
    //         options.push({label: this.reportDate, value: this.reportDate});
    //         return options;
    //     }
    //     for (let i = 0; i < this.reportDateOptionList.length; i++) {
    //         var dateOption = this.reportDateOptionList[i];
    //         options.push({label: this.sevenDaysFormat(dateOption), value: dateOption});
    //     }
    //     return options;
    // };
    // sevenDaysFormat(setDateStr) {
    //     // var startDate = new Date(setDateStr);
    //     var endDateNumber = new Date(setDateStr).getTime()+6*24*60*60*1000;
    //     var endDate = new Date(endDateNumber);
    //     var year = endDate.getFullYear();
    //     var month = endDate.getMonth() + 1;
    //     var day = endDate.getDate();
    //     month = month<10 ? '0'+month : month;
    //     day = day<10 ? '0'+day : day;      
    //     return setDateStr+' ~ '+year+'-'+month+'-'+day;
    // }
        ...
        judgeCountry().then(data => {
            ...
            if (data.isSuccess) {
                ...
                if (data.data.SouthAfrica) {
                    ...
                    this.showWarning(To_New_Page.format(Weekly_Report_Title));
    ...
    saveReportCore(isSubmit, type) {
        ...
        if (checkResp.alltrue == false) {
            this.showWarning(this.label.PromoterDailyReport_RequiredCheck + checkResp.msg);
            this.lwcName = this.label.PromoterDailyReport_Market_Report;
            return;
        }
    ...
    */
                ...
                for (let j = 0; j < intelligence.length; j++) {
                    ...
                    if (j == 0) {
                        if (info.Product__c == null || info.Product__c == '') {
                            resp.alltrue = false;
                            resp.msg = this.intelligenceFields.Product__c;
                            this.lwcName = this.label.PromoterDailyReport_INTELLIGENCE;
                            return resp;
                        }
                    } else {
                        if (info.ProductInformation__c == null || info.ProductInformation__c == '') {
                            resp.alltrue = false;
                            resp.msg = this.intelligenceFields.ProductInformation__c;
                            this.lwcName = this.label.PromoterDailyReport_INTELLIGENCE;
                            return resp;
                        }
                    }
                    if (info.Brands__c == null || info.Brands__c == '') {
                        resp.alltrue = false;
                        resp.msg = this.intelligenceFields.Brands__c;
                        this.lwcName = this.label.PromoterDailyReport_INTELLIGENCE;
                        return resp;
                    } else if (info.NewProduct__c == null || info.NewProduct__c == '') {
                        resp.alltrue = false;
                        resp.msg = this.intelligenceFields.NewProduct__c;
                        this.lwcName = this.label.PromoterDailyReport_INTELLIGENCE;
                        return resp;
                    }
                }
    ...
    handleReportChangeCombobox(event) {
        console.log('event.target.fieldName' + event.target.fieldName);
        console.log('event.target.value' + event.target.value);
    ...
    handleChangeIntelligenceOption(event) {
        ...
        if (fieldName == 'Brands__c') {
            if (selectRecord == undefined) {
                this.intelligenceList[Number(index)].Item[Number(idx)].Brands__c = null;
            } else {
                this.intelligenceList[Number(index)].Item[Number(idx)].Brands__c = selectRecord.Id;
                console.log('intelligenceList>>>brand>>>' + this.intelligenceList[Number(index)].Item[Number(idx)].Brands__c);
            }
        }
        if (fieldName == 'Product__c') {
            if (selectRecord == undefined) {
