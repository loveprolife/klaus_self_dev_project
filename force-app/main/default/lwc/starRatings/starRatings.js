...
import initData from '@salesforce/apex/StarRatingController.initData';
export default class StarRating extends LightningElement {
    ...
    connectedCallback() {
        ...
        initData({
            ...
            // objectName : 'Customer_Visit__c'
        }).then(result => {
            this.setRating(result);
            this.oldRatingValue = result;
        }).catch(error => {
    ...
    onStarClicked(event) {
        ...
        updateRating({
            ...
            oldRatingValue : this.oldRatingValue,
        }).then(result => {
            if(result){
                if(ratingValue != this.oldRatingValue){
                    this.oldRatingValue = ratingValue;
                }else {
                    this.oldRatingValue = '0';
                }   
                this.setRating(this.oldRatingValue); 
            }
